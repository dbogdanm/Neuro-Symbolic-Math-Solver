...
SYSTEM_PROMPT_TEMPLATE = 'Esti un asistent matematic precis. Rezolvă problema pas cu pas.\n\n\n\n{hint_bloc}\n\n\n\nREGULI OBLIGATORII DE FORMATARE A RĂSPUNSULUI:\n\n1. Poți folosi tag-urile <think>...</think> pentru raționament intern.\n\n2. ULTIMA linie din răspunsul tău TREBUIE să fie EXACT în formatul:\n\n   RASPUNS_FINAL: <valoarea_numerică_sau_expresia_simplificată>\n\n\n\n   Exemple corecte:\n\n   RASPUNS_FINAL: 42\n\n   RASPUNS_FINAL: -3/4\n\n   RASPUNS_FINAL: x=5 sau x=-2\n\n   RASPUNS_FINAL: (2, 7)\n\n\n\n3. Nu adăuga NIMIC după linia RASPUNS_FINAL.\n\n4. Nu scrie "RASPUNS_FINAL:" de mai multe ori.\n\n\n\nPROBLEMA:\n\n{problema}\n\n'
...
def extrage_layer2_dupa_think(text: str) -> str | None:
    ...
    patterns = ['x\\s*=\\s*[-\\d.,/\\s]+(?:sau|or|și|and|,)\\s*x\\s*=\\s*[-\\d.,/]+', 'x\\s*=\\s*[-\\d.,/]+', '\\(\\s*[-\\d.,/]+\\s*,\\s*[-\\d.,/]+\\s*\\)', '[-]?\\d+\\s*/\\s*\\d+', '[-]?\\d+[.,]\\d+', '[-]?\\d+']
    for pat in patterns:
...
def extrage_layer3_llm_extractor(text_brut: str, ollama_url: str='http://localhost:11434/api/generate', model: str='deepseek-r1:8b') -> str | None:
    prompt_extractor = f'Din textul de mai jos, extrage DOAR valoarea numerică finală sau soluția matematică finală.\n\nRăspunde cu UN SINGUR RÂND în formatul: RASPUNS_FINAL: <valoare>\n\nNu adăuga explicații.\n\n\n\nTEXT:\n\n{text_brut[:2000]}  \n\n'
